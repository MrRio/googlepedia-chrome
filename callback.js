...
function getFeed(feed,callback) {
	...
	//console.log(feed);
	//alert('hi');
	xhr.open("GET", 'http://en.wikipedia.org/wiki/Moose', true);
	xhr.onreadystatechange = function(data) {
	  if (xhr.readyState == 4) {
		console.log(data);
		
		console.log(xhr.responseText);
		wikibox.innerHTML = xhr.responseText;
		//urlCallback(xhr.responseText);

		var url = wikibox.getElementsByTagName('h3')[0];
		var url = url.getElementsByTagName('a')[0].href;
		//console.log(url);
...
if(getValue('hidden', '0') == '0' && (start == '' || start == 0)) {
	initWikibox();
}
...
window.urlCallback = function(data) {
				articleLoaded = true;
			wikibox.innerHTML = data.content;
