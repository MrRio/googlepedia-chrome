...
function getFeed(feed,callback) {
	chrome.extension.sendRequest({'action' : 'fetchWikipediaArticle', 'url' : feed}, function(data) {
		urlCallback(data);
	});
...
function getValue(name, d) {
    if(name == 'rewrite_links') {
        return true;
    }
    if(name == 'language') {
        return '';
    }
	return readCookie(name, d, 365);
}

function setValue(key, val) {
    //alert('Set value ' + key + '=' + val);
	return createCookie(key, val, 365);
}
...
if (getValue('language') == '') {
	...
	wikipediaLanguage = getValue('language');
}
...
function retrieveURL(search_url) {
    url = search_url;
	getFeed(search_url,'urlCallback');
}


addGlobalStyle('#wikibox { z-index: 999; background-color: #fff; overflow: hidden; position: relative; top: 20px;	} #imagebox { float: left;} li.g { clear:left}');
...
if(getValue('hidden', '0')=='1' || (start != '' && start != '0')) {
...
//alert(getValue('hidden', '0'));
if(getValue('hidden', '0') == '0' && (start == '' || start == 0) && (/http:\/\/www\.google\..*\/search.*q=.*/.test(document.location.href))) {
	initWikibox();
}
...
function getString(key) {
    var en_US = {
        noArticleFound:'Sorry, there was no relevant article found.',
        viewOnWikipedia:'View on Wikipedia',
        loadingArticlePleaseWait:'Loading Wikipedia article. Please wait...',
        networkProblem:'There was a problem connecting to Wikipedia.',
        hide:'Hide',
        expand:'Expand',
        shrink:'Shrink'
    }
...
var urlCallback = function(data) {
				articleLoaded = true;
			wikibox.innerHTML = data;
			...
			var rewriteLinks = getValue('rewrite_links');
			...
			for (var i = 0; i < allLinks.length; i++) {
				...
				if(thisHref.match('Image:')) {
					...
					if(!thisHref.match(/http:\/\/(.*).google.(.*)\/wiki\/#/)) {
                        ...
                        thisHref = thisHref.replace(/http:\/\/(.*).google.(.*)\/wiki\/(.*)#(.*)/, document.location.href + "#$4");
                    }
					//Internal link
				} else if(thisHref.match('/w/index.php')) {
					thisHref = thisHref.replace(/http:\/\/(.*).wikipedia.(.*)\/w\//,wikipediaUrl+"w/")
				} else {
					if(rewriteLinks) {
